        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (attempt === maxRetries) {
                    throw new Error(`Failed to connect to Temporal after ${maxRetries} attempts: ${error.message}`);
                }
                console.warn(`[Temporal] Connection attempt ${attempt}/${maxRetries} failed, retrying in ${retryDelay}ms...`);
...
// ── Mock Supplier A ───────────────────────────────────────────────────────────
app.get("/supplierA/hotels", (_req, res) => {
    const { city } = _req.query;
    const cityStr = city?.toLowerCase().trim();
    if (!cityStr || !config_1.config.knownCities.includes(cityStr)) {
        return res.json([]);
    }
    const cityData = {
        delhi: [
...
// ── Mock Supplier B ───────────────────────────────────────────────────────────
app.get("/supplierB/hotels", (_req, res) => {
    const { city } = _req.query;
    const cityStr = city?.toLowerCase().trim();
    if (!cityStr || !config_1.config.knownCities.includes(cityStr)) {
        return res.json([]);
...
app.get("/health", async (_req, res) => {
    ...
    try {
        await axios_1.default.get(`${baseUrl}/supplierA/hotels?city=${config_1.config.healthCheckCity}`, { timeout: config_1.config.healthCheckTimeoutMs });
        status.supplierA = "up";
    }
    catch {
    ...
    try {
        await axios_1.default.get(`${baseUrl}/supplierB/hotels?city=${config_1.config.healthCheckCity}`, { timeout: config_1.config.healthCheckTimeoutMs });
        status.supplierB = "up";
    }
    catch {
...
app.get("/api/hotels", async (req, res) => {
    ...
    // Validate city
    if (cityKey.length > config_1.config.maxCityLength ||
        !config_1.config.cityRegex.test(cityKey)) {
        res
            .status(400)
            .json({
            error: "Invalid city parameter. Must be alphanumeric with spaces/hyphens only.",
        });
        return;
    }
    ...
    try {
        ...
        if (cacheExists) {
            ...
            const cached = await redisClient.zRangeByScore(`hotels:${cityKey}`, min, max);
            if (cached.length > 0) {
                console.log(`[Server] Cache HIT for "${cityKey}" — ${cached.length} results.`);
        ...
        console.log(`[Server] Cache MISS for "${cityKey}" — starting Temporal workflow.`);
        const client = await getTemporalClient();
        const result = await client.workflow.execute(workflows_1.hotelAggregationWorkflow, {
            args: [cityKey],
            taskQueue: config_1.config.temporalTaskQueue,
            workflowId: `${config_1.config.temporalWorkflowIdPrefix}-${cityKey}-${Date.now()}`,
        });
...
// ── Start ─────────────────────────────────────────────────────────────────────
app
    .listen(config_1.config.port, () => console.log(`[Server] Running on http://localhost:${config_1.config.port}`))
    .on("error", (err) => {
